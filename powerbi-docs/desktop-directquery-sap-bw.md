...
* De maneira semelhante, não há nenhum **Editor de Consultas** ao se conectar ao SAP BW. As configurações da fonte de dados (por exemplo, o nome do servidor) podem ser alteradas selecionando **Editar Consultas > Configurações da fonte de dados**. As configurações de todas as variáveis podem ser alteradas selecionando **Editar Consultas > Editar Variáveis**.
